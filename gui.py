    ...
    def arrange_widgets(self):
        ...
        for y in range(9):
            for x in range(9):
                ...
                self.lbl_arr[x][y] = Label(
                    self.window, width=2, font=Font(size=15), bg="gray", fg="white"
                )

        self.ok = IntVar()
        self.ok_btn = Button(
            self.window, text="OK", width=40, height=1, command=lambda: self.ok.set(1)
        )
        self.ok_btn.grid(row=13, column=0, columnspan=9)
        self.ok_btn.grid_forget()

        self.addnum_btn = Button(
            self.window, text="Add Number", width=40, height=1, command=self.add_board
        )
        self.addnum_btn.grid(row=13, column=0, columnspan=9)

        self.sovle_btn = Button(
            self.window, text="Solve", width=40, height=1, command=self.solve
        )
        self.sovle_btn.grid(row=13, column=0, columnspan=9)
        self.sovle_btn.grid_forget()

        self.wait_btn = Button(self.window, text="Wait..", width=40, height=1)
        self.wait_btn.grid(row=13, column=0, columnspan=9)
